...
-- PÁGINA JOGADOR
local pJog = criarPagina("Jogador")
...
local rastroConn = nil
local rastroObj = nil

-- Função: pega o Character atual
...
local function aplicarCoroa()
    ...
    bb.Name = "DyvCrown"
    bb.Size = UDim2.fromOffset(60, 40)
    ...
    task.spawn(function()
        local hue = 0
        while coroaAtiva and bb.Parent do
            hue = (hue + 0.01) % 1
            lbl.TextColor3 = Color3.fromHSV(hue, 0.6, 1)
            task.wait(0.05)
        end
    end)
end
...
local function removerRastro()
    if rastroConn then rastroConn:Disconnect(); rastroConn = nil end
...
criarBotao3D(pJog, "🌀 Aura (Amarelo/Ciano)", Color3.fromRGB(120,100,30), function()
    if auraAtiva then removerAura() else aplicarAura() end
end, 1)

criarBotao3D(pJog, "🌿 Rastro de Grama", Color3.fromRGB(40,120,50), function()
    if gramaAtiva then removerGrama() else aplicarGrama() end
end, 2)

criarBotao3D(pJog, "👑 Coroa Flutuante", Color3.fromRGB(150,120,30), function()
    if coroaAtiva then removerCoroa() else aplicarCoroa() end
end, 3)

criarBotao3D(pJog, "💨 Rastro Visual", Color3.fromRGB(60,100,140), function()
    if rastroAtivo then removerRastro() else aplicarRastro() end
end, 4)

criarBotao3D(pJog, "🚫 Remover Tudo", Color3.fromRGB(90,30,30), function()
    removerAura()
    removerGrama()
    removerCoroa()
    removerRastro()
...
-- PÁGINA AMBIENTE
local pAmb = criarPagina("Ambiente")

local chuvaAtiva = false
local chuvaConn = nil
local neblinaAtiva = false
local neblinaConn = nil
local auroraAtiva = false
local auroraConn = nil
local estrelasAtiva = false
local estrelasConn = nil
local relampagoAtivo = false
local relampagoConn = nil

-- CHUVA DE METEOROS
local function removerMeteoros()
    if chuvaConn then chuvaConn:Disconnect(); chuvaConn = nil end
    chuvaAtiva = false
end
...
local function aplicarMeteoros()
    removerMeteoros()
    chuvaAtiva = true
    chuvaConn = RunService.Heartbeat:Connect(function()
        if not chuvaAtiva then return end
...
local function removerEstrelas()
    if estrelasConn then estrelasConn:Disconnect(); estrelasConn = nil end
    estrelasAtiva = false
end
...
local function aplicarEstrelas()
    ...
    if not root then return end
    estrelasAtiva = true
    estrelasConn = RunService.Heartbeat:Connect(function()
        if not estrelasAtiva then return end
...
local function removerRelampago()
    if relampagoConn then relampagoConn:Disconnect(); relampagoConn = nil end
    relampagoAtivo = false
    Lighting.Brightness = ceuBackupSky.Brightness
end
...
local function aplicarRelampago()
    removerRelampago()
    relampagoAtivo = true
    relampagoConn = RunService.Heartbeat:Connect(function()
        if not relampagoAtivo then return end
        if math.random() < 0.01 then
            Lighting.Brightness = 10
            task.delay(0.08, function()
                if relampagoAtivo then
                    Lighting.Brightness = ceuBackupSky.Brightness
                end
            end)
        end
...
criarBotao3D(pAmb, "☄️ Chuva de Meteoros", Color3.fromRGB(150,80,20), function()
    if chuvaAtiva then removerMeteoros() else aplicarMeteoros() end
end, 1)

criarBotao3D(pAmb, "🌫️ Neblina Colorida", Color3.fromRGB(80,80,120), function()
    if neblinaAtiva then removerNeblina() else aplicarNeblina() end
end, 2)

criarBotao3D(pAmb, "🌌 Aurora Boreal", Color3.fromRGB(60,120,180), function()
    if auroraAtiva then removerAurora() else aplicarAurora() end
end, 3)

criarBotao3D(pAmb, "⭐ Chuva de Estrelas", Color3.fromRGB(150,150,80), function()
    if estrelasAtiva then removerEstrelas() else aplicarEstrelas() end
end, 4)

criarBotao3D(pAmb, "⚡ Relâmpagos", Color3.fromRGB(120,120,40), function()
    if relampagoAtivo then removerRelampago() else aplicarRelampago() end
end, 5)

criarBotao3D(pAmb, "🚫 Remover Tudo", Color3.fromRGB(90,30,30), function()
    removerMeteoros()
    removerNeblina()
    removerAurora()
    removerEstrelas()
...
criarBotaoSidebar("Jogador", "👤", 5)
criarBotaoSidebar("Ambiente", "🌍", 6)
criarBotaoSidebar("Música", "🎵", 7)
criarBotaoSidebar("Config", "⚙", 8)
mostrarPagina("FPS")
...
print("[DYV HUB v0.7] Carregado com sucesso!")0
