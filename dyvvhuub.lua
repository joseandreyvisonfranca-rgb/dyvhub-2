--==================================================
-- DYV HUB v2.2 - MOBILE EDITION
-- 11 Abas + Loading + Música Minecraft + Aba Mapa
--==================================================
...
local ICONE = "rbxassetid://103731828151465"
...
-- SOM AUTOMÁTICO (Minecraft)
local somFundo = Instance.new("Sound")
somFundo.Name = "DYV_Music"
somFundo.SoundId = "rbxassetid://107745387481761"
...
-- LOADING (1 a 100% em 10 segundos)
local loadingFrame = Instance.new("Frame")
loadingFrame.Size = UDim2.fromScale(1, 1)
loadingFrame.BackgroundColor3 = BLACK
loadingFrame.ZIndex = 998
loadingFrame.Parent = gui

local loadingText = Instance.new("TextLabel")
loadingText.Size = UDim2.fromScale(1, 0, 0, 100)
loadingText.Position = UDim2.fromScale(0, 0.5)
loadingText.AnchorPoint = Vector2.new(0, 0.5)
loadingText.BackgroundTransparency = 1
loadingText.Text = "Carregando... 0%"
loadingText.TextColor3 = GREEN
loadingText.TextScaled = true
loadingText.Font = FONTE_BOLD
loadingText.ZIndex = 999
loadingText.Parent = loadingFrame

local loadingBarBg = Instance.new("Frame")
loadingBarBg.Size = UDim2.fromOffset(300, 8)
loadingBarBg.Position = UDim2.new(0.5, -150, 0.5, 80)
loadingBarBg.BackgroundColor3 = BLACK
loadingBarBg.BorderSizePixel = 0
loadingBarBg.ZIndex = 999
...
loadingBarFill.BackgroundColor3 = GREEN
loadingBarFill.BorderSizePixel = 0
loadingBarFill.ZIndex = 1000
...
-- ANIMAÇÃO DE 0 A 100 EM 10 SEGUNDOS
task.spawn(function()
	for i = 0, 100 do
...
-- SISTEMA DE POSIÇÕES SALVAS
local posicoesSalvas = {}

local function salvarPosicao(nome)
...
local function teleportarParaPosicao(nome)
	local char = player.Character
	if not char then return end
	local root = char:FindFirstChild("HumanoidRootPart")
	if not root then return end

	for _, pos in ipairs(posicoesSalvas) do
		if pos.nome == nome then
			root.CFrame = pos.posicao
			return true
		end
	end
	return false
...
local function deletarPosicao(nome)
...
local function buildMapa()
	...
	scrollMap.Size = UDim2.new(1, 0, 1, 0)
	scrollMap.BackgroundTransparency = 1
	scrollMap.BorderSizePixel = 0
	scrollMap.ScrollBarThickness = 4
	scrollMap.ScrollBarImageColor3 = GREEN
	scrollMap.CanvasSize = UDim2.new(0, 0, 0, 0)
	scrollMap.AutomaticCanvasSize = Enum.AutomaticSize.Y
	scrollMap.Parent = contentArea

	local listMap = Instance.new("UIListLayout")
	listMap.Padding = UDim.new(0, 5)
	listMap.SortOrder = Enum.SortOrder.LayoutOrder
	listMap.Parent = scrollMap

	local salvarBtn, salvarLbl = makeBtn3D(scrollMap, "SALVAR POSIÇÃO ATUAL", "Guarda onde você tá agora")
	local irBtn, irLbl         = makeBtn3D(scrollMap, "IR PARA POSIÇÃO", "Teleporta pra uma salva")
	local listaBtn, listaLbl   = makeBtn3D(scrollMap, "MOSTRAR POSIÇÕES", "Mostra as posições salvas")
	local antiVoidBtn, antiVoidLbl = makeBtn3D(scrollMap, "ANTI-VOID", "Não deixa você cair do mapa")

	registrarBotao("SALVAR POSIÇÃO ATUAL", "Guarda onde você tá agora", salvarBtn)
	registrarBotao("IR PARA POSIÇÃO", "Teleporta pra uma salva", irBtn)
	registrarBotao("MOSTRAR POSIÇÕES", "Mostra as posições salvas", listaBtn)
	registrarBotao("ANTI-VOID", "Não deixa você cair do mapa", antiVoidBtn)

	-- TÍTULO DA LISTA
	local listaTitulo = Instance.new("TextLabel")
	listaTitulo.Size = UDim2.new(1, 0, 0, 20)
	listaTitulo.BackgroundTransparency = 1
	listaTitulo.Text = "📋  POSIÇÕES SALVAS"
	listaTitulo.TextColor3 = GREEN
	listaTitulo.TextSize = 11
	listaTitulo.Font = FONTE_BOLD
	listaTitulo.TextXAlignment = Enum.TextXAlignment.Left
	listaTitulo.Visible = false
	listaTitulo.Parent = scrollMap

	local listaFrame = Instance.new("Frame")
	listaFrame.Size = UDim2.new(1, -6, 0, 0)
	listaFrame.AutomaticSize = Enum.AutomaticSize.Y
	listaFrame.BackgroundColor3 = BLACK
	listaFrame.BackgroundTransparency = 0.3
	listaFrame.BorderSizePixel = 0
	listaFrame.Visible = false
	listaFrame.Parent = scrollMap

	local listaCorner = Instance.new("UICorner")
	listaCorner.CornerRadius = UDim.new(0, 10)
	...
	listaPad.PaddingTop = UDim.new(0, 6)
	listaPad.PaddingBottom = UDim.new(0, 6)
	listaPad.PaddingLeft = UDim.new(0, 6)
	listaPad.PaddingRight = UDim.new(0, 6)
	listaPad.Parent = listaFrame

	local listaVisivel = false
	...
	local function atualizarLista()
		...
		local posicoes = listarPosicoes()

		if #posicoes == 0 then
			local vazio = Instance.new("TextLabel")
			vazio.Size = UDim2.new(1, 0, 0, 25)
			vazio.BackgroundTransparency = 1
			vazio.Text = "Nenhuma posição salva"
			vazio.TextColor3 = Color3.fromRGB(150, 150, 150)
			vazio.TextSize = 11
			vazio.Font = FONTE
			vazio.LayoutOrder = 1
			vazio.Parent = listaFrame
			return
		end
		...
		for i, pos in ipairs(posicoes) do
			...
			row.MouseButton1Click:Connect(function()
				teleportarParaPosicao(pos.nome)
				row.Text = "   ✅ Teleportado!"
				task.wait(1.5)
				row.Text = "   📍 " .. pos.nome
			end)
		end
	...
	salvarBtn.MouseButton1Click:Connect(function()
		...
		input.BackgroundColor3 = BLACK
		input.TextColor3 = GREEN
		input.PlaceholderText = "Ex: Casa, Loja..."
		input.PlaceholderColor3 = Color3.fromRGB(130,130,130)
		input.TextSize = 13
		input.Font = FONTE
		input.ClearTextOnFocus = false
		...
		confirm.MouseButton1Click:Connect(function()
			local nome = input.Text
			if nome == "" then nome = "Posição " .. (#posicoesSalvas + 1) end
			salvarPosicao(nome)
			panel:Destroy()
			salvarLbl.Text = "SALVO!"
			task.wait(1.5)
			salvarLbl.Text = "SALVAR POSIÇÃO ATUAL"
		end)
	end)

	-- BOTÃO IR PARA POSIÇÃO
	irBtn.MouseButton1Click:Connect(function()
		local posicoes = listarPosicoes()
		if #posicoes == 0 then
			irLbl.Text = "NENHUMA SALVA"
			task.wait(1.5)
			irLbl.Text = "IR PARA POSIÇÃO"
			return
		end
		...
		local listaPanel = Instance.new("ScrollingFrame")
		listaPanel.Size = UDim2.new(1, -20, 1, -50)
		listaPanel.Position = UDim2.fromOffset(10, 35)
		listaPanel.BackgroundColor3 = BLACK
		listaPanel.BackgroundTransparency = 0.3
		listaPanel.BorderSizePixel = 0
		listaPanel.ScrollBarThickness = 4
		listaPanel.ScrollBarImageColor3 = GREEN
		listaPanel.CanvasSize = UDim2.new(0, 0, 0, 0)
		listaPanel.AutomaticCanvasSize = Enum.AutomaticSize.Y
		listaPanel.ZIndex = 101
		listaPanel.Parent = panel

		local lc = Instance.new("UICorner"); lc.CornerRadius = UDim.new(0, 10); lc.Parent = listaPanel

		local ll = Instance.new("UIListLayout")
		ll.Padding = UDim.new(0, 4)
		ll.Parent = listaPanel
	...
	-- BOTÃO MOSTRAR POSIÇÕES
	listaBtn.MouseButton1Click:Connect(function()
		listaVisivel = not listaVisivel
		listaTitulo.Visible = listaVisivel
		listaFrame.Visible = listaVisivel

		if listaVisivel then
			atualizarLista()
			listaLbl.Text = "ESCONDER POSIÇÕES"
		else
			listaLbl.Text = "MOSTRAR POSIÇÕES"
		end
	end)
	...
	local function removerAntiVoid()
		if antiVoidConn then antiVoidConn:Disconnect(); antiVoidConn = nil end
		antiVoidAtivo = false
	end

	local function aplicarAntiVoid()
		removerAntiVoid()
		antiVoidAtivo = true
	...
	antiVoidBtn.MouseButton1Click:Connect(function()
		if antiVoidAtivo then
			removerAntiVoid()
			antiVoidLbl.Text = "ANTI-VOID"
		else
			aplicarAntiVoid()
			antiVoidLbl.Text = "ANTI-VOID: ON"
		end
	end)
end

--==================================================
-- BOTÕES DO SIDEBAR (11 ABAS)
--==================================================
...
createSidebarButton("Jogador", 34, function()
	selectTab(2, "JOGADOR")
	buildPlayer()
end)

createSidebarButton("Funções", 62, function()
	selectTab(3, "FUNÇÕES")
	buildFunctions()
end)

createSidebarButton("Mira", 90, function()
	selectTab(4, "MIRA E APELAÇÕES")
	buildMira()
end)

createSidebarButton("Efeitos", 118, function()
	selectTab(5, "EFEITOS")
	buildEfeitos()
end)

createSidebarButton("Ambiente", 146, function()
	selectTab(6, "AMBIENTE")
	buildAmbiente()
end)

createSidebarButton("Mapa", 174, function()
	selectTab(7, "MAPA")
	buildMapa()
end)

createSidebarButton("Música", 202, function()
	selectTab(8, "MÚSICA")
	buildMusic()
end)

createSidebarButton("Preferências", 230, function()
	selectTab(9, "PREFERÊNCIAS")
	buildPreferences()
end)

createSidebarButton("Outros", 258, function()
	selectTab(10, "OUTROS")
	buildOutros()
end)

createSidebarButton("Créditos", 286, function()
	selectTab(11, "CRÉDITOS")
	buildCredits()
end)
